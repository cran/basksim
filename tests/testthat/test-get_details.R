...
test_that("get_details works for exnex", {
  ...
  # If n is passed as a vector, it should have k entries
  expect_error(get_details.exnex(
    design = design,
    n = c(10, 20),
    p1 = NULL,
    lambda = 0.95,
...
test_that("get_details works for fujikawa", {
  ...
  expect_equal(
    res1$Rejection_Probabilities,
    c(0.1003108, 0.1003108, 0.5965844),
    tolerance = tol
  )
  ...
  # If n is passed as a vector, it should have k entries
  expect_error(get_details.fujikawa(
    design = design1,
    n = c(10, 20),
    p1 = NULL,
    lambda = 0.95,
  ...
  # Compare with results from baskexact
  expect_equal(
    res2$Rejection_Probabilities,
    c(0.3133054, 0.7274739, 0.9793341, 0.3133054),
    tolerance = tol
  )
  expect_equal(res2$FWER, 0.4696476, tolerance = tol)
  expect_equal(res2$EWP, 0.9850177, tolerance = tol)
  expect_equal(
    res2$Mean,
    c(0.2145436, 0.3134261, 0.4571671, 0.2145436),
    tolerance = tol
  )
  expect_equal(
    res2$MSE,
    c(0.009882112, 0.009797917, 0.016591157, 0.009882112),
    tolerance = tol
  )
  expect_equal(res2$ECD, 3.080197, tolerance = tol)

  # Check MCSEs
  expect_equal(res2$FWER_SE, sqrt((res2$FWER) * (1 - res2$FWER) / n_iter))
  expect_equal(res2$EWP_SE, sqrt((res2$EWP) * (1 - res2$EWP) / n_iter))
  expect_equal(
    res2$Rejection_Probabilities_SE,
    sqrt(
      (res2$Rejection_Probabilities) *
        (1 - res2$Rejection_Probabilities) /
        n_iter
    )
  )
  expect_true(res2$ECD_SE < 1 / sqrt(n_iter))
  expect_true(res2$ECD_SE > 0.001 / sqrt(n_iter))
})
...
test_that("get_details works for fujikawa with custom weights functions", {
  ...
  rescustom <- get_details(
    ...
    tau = tau,
    iter = iter,
    weight_fun = weight_noshare
  )
  # Compare with results with sharing - sharing should increase power and TOER
  expect_true(all(
    rescustom$Rejection_Probabilities < res$Rejection_Probabilities
  ))
...
test_that("get_details works for jsdglobal", {
  ...
  expect_equal(
    res1$Rejection_Probabilities,
    c(0.3500560, 0.8917081, 0.9701317),
    tolerance = tol
  )
  ...
  expect_equal(
    res2$Rejection_Probabilities,
    c(0.2070795, 0.7676506, 0.5765870, 0.8949090),
    tolerance = tol
  )
  expect_equal(
    res2$Mean,
    c(0.2374997, 0.3846106, 0.3261889, 0.4454463),
    tolerance = tol
  )
  expect_equal(
    res2$MSE,
...
test_that("get_details works for cpp", {
  ...
  expect_equal(
    res1$Rejection_Probabilities,
    c(0.06643573, 0.06643573, 0.56254586),
    tolerance = tol
  )
  expect_equal(res1$Mean, c(0.2529584, 0.2529584, 0.4173126), tolerance = tol)
  expect_equal(
    res1$MSE,
    c(0.008506501, 0.008506501, 0.018349713),
    tolerance = tol
  )
  ...
  expect_equal(
    res2$Rejection_Probabilities,
    c(0.05833261, 0.05833261, 0.05833261, 0.40165309),
    tolerance = tol
  )
  expect_equal(res2$FWER, 0.1142539, tolerance = tol)
  expect_equal(
    res2$Mean,
    c(0.1938748, 0.1938748, 0.1938748, 0.2966433),
    tolerance = tol
  )
  expect_equal(
    res2$MSE,
    c(0.005062454, 0.005062454, 0.005062454, 0.017710313),
    tolerance = tol
  )
  expect_equal(res2$ECD, 3.226655, tolerance = tol)
  # If n is passed as a vector, it should have k entries
  expect_error(get_details.cpp(
    design = design1,
    n = c(10, 20),
    p1 = NULL,
    lambda = 0.95,
...
test_that("get_details works for cppglobal", {
  ...
  expect_equal(
    res1$Rejection_Probabilities,
    c(0.3270933, 0.8168261, 0.9432173),
    tolerance = tol
  )
  expect_equal(res1$Mean, c(0.2764610, 0.3967091, 0.4596072), tolerance = tol)
  expect_equal(
    res1$MSE,
  ...
  expect_equal(
    res2$Rejection_Probabilities,
    c(0.2270686, 0.5359827, 0.8157377, 0.9551109),
    tolerance = tol
  )
  expect_equal(
    res2$Mean,
    c(0.2232156, 0.2794798, 0.3387829, 0.4002807),
    tolerance = tol
  )
  expect_equal(
    res2$MSE,
    c(0.008987673, 0.005730799, 0.006345472, 0.010459698),
    tolerance = tol
  )
  expect_equal(res2$ECD, 3.079763, tolerance = tol)
})
...
test_that("get_details works for mml", {
  ...
  expect_equal(
    res1$Rejection_Probabilities,
    c(0.2317285, 0.7058685, 0.9801984),
    tolerance = tol
  )
  expect_equal(res1$Mean, c(0.2606807, 0.4056437, 0.5567046), tolerance = tol)
  expect_equal(res1$ECD, 2.454338, tolerance = tol)
  ...
  expect_equal(res2$FWER, 0.3914886, tolerance = tol * 2)
  expect_equal(
    res2$Mean,
  ...
  expect_equal(
    res2$MSE,
    c(0.006931833, 0.006931833, 0.010724183, 0.010724183),
    tolerance = tol
  )
  expect_equal(res2$ECD, 3.32808, tolerance = tol)
})
...
test_that("get_details works for cpplim", {
  ...
  res_cpplim <- get_details.cpplim(
    design = design_cpplim,
    n = n,
    p1 = p1,
    lambda = lambda,
    tune_a = 1,
  ...
  expect_equal(
    res_cpplim$Rejection_Probabilities,
    res_cpp$Rejection_Probabilities
  )
  ...
  # If n is passed as a vector, it should have k entries
  expect_error(get_details.cpplim(
    design = design_cpplim,
    n = c(10, 20),
    p1 = NULL,
    lambda = 0.95,
...
test_that("CPP and LCPP work", {
  ...
  expect_equal(
    res_cpp$Rejection_Probabilities[3],
    res_cpplim$Rejection_Probabilities[3]
  )
})
